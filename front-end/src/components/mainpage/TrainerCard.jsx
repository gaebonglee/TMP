...
const TrainerCard = () => {
  ...
  const trainersToShow = data.info.slice(currentIndex, currentIndex + numCardsToShow);

  return (
    <div className='ai_wrapper'>
          <IoIosArrowBack onClick={prev} className={`arrow left ${currentIndex <= 0 ? "disabled" : ""}`}/>
    <section className="ai_container">
      <div>
          ...
          {trainersToShow.map((v, i) => (
            <li key={i} className={`animate-${animationDirection}`}>
              <div className="ai_contents">
                <Link to={`/trainerDetail/${v.user_id}`} className="ai_contents_a">
        ...
        {currentIndex + numCardsToShow < data.info.length && (
          <IoIosArrowForward onClick={next} className="arrow right"/>
        )}
    </div>
  );
};

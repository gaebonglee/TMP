...
function selectCoach(user_id) {
  ...
  console.log(user_id)
  const sql = `SELECT u.user_name, u.email, u.phonenumber, u.user_id, t.trainning_type, c.center_name, c.center_id FROM user u JOIN trainer t ON u.user_id = t.user_id JOIN center c ON t.center_id = c.center_id where u.user_id = ?`;
